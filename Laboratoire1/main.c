...
int main(void)
{
	int M = valider_entier("Entrez le nombre de lignes  [4..10] : ", 4, 10);
	int N = valider_entier("Entrez le nombre de colonnes [4..18] : ", 4, 18);
	...
	t_pile chemins_W = init_pile(2*M * N); //initialiser la pile des chemins de Wilson
	t_pile longueurs_W = init_pile(M * N); //initialiser la pile des longueurs de Wilson
    
	etape_des_2piles(&chemins_W, &longueurs_W, M, N); //créer les piles de Wilson pour le labyrinthe de taille nb_lig(M) x nb_col(N)
	...
	//erreur commence ici
	t_labyrinthe laby = init_laby(M, N); //initialiser le labyrinthe de taille nb_lig(M) x nb_col(N)
	etape_ouverture_chemins(&laby, &chemins_W, &longueurs_W); //ouvrir les chemins de Wilson dans le labyrinthe

	int contour1 = mt_randi(4); //choisir un contour au hasard pour creuser une porte
	creuser_une_porte(&laby, contour1); //creuser une porte dans le contour choisi

	int contour2;
	do {
		contour2 = mt_randi(4); //choisir un autre contour au hasard pour creuser une porte
	} while (contour1 == contour2);
	creuser_une_porte(&laby, contour2); //creuser une porte dans le second contour choisi
	...
	detruire_laby(&laby); //libérer la mémoire allouée pour le labyrinthe
